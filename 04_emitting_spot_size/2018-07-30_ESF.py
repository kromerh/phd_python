...
start_date = "2018-07-30 10:50:00"
my_date = re.findall(r'(\d\d\d\d-\d\d-\d\d) ', start_date)[0]
# end of measurement
end_date = "2018-07-30 15:36:00"

# When the background measurement started and ended in a list
lst_bg_start = ['09:18:00', '15:38:00']
lst_bg_end = ['09:59:00', '16:38:30']
...
df1 = df[ df['t'] < datetime.datetime.strptime('13:34:20', FMT).time() ]
df2 = df[ df['t'] > datetime.datetime.strptime('13:41:54', FMT).time() ]
# print(df1.head(20))
# print(df2.tail(20))
...
for df in [df1, df2]:
	...
	def getAverageDose(t_start, t_end, df_dose):
		...
		str_start = datetime.time.strftime(t_start, '%H:%M:%S').replace(':','-')
		# plotXY(my_df['t'].values[::10].astype(str), my_df['dose_corrected'].valuesvalues[::10].astype(float), 'time', 'Dose corrected', f'Dose \n at starttime {t_start}', f'dose_{str_start}')
		# print(t_start, t_end)
...
df_result = df_result[df_result.cps > 0]
