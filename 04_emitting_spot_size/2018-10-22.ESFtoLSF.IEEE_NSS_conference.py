...
ax.scatter(df.edge_pos_mm_centered, df.norm_cps, color='darkorange', label='Measurement')
ax.plot(x,y_fit, color='darkblue', label='Logistic fit')
ax.set_xlabel('Edge position [mm]', fontsize=14, labelpad=10)
ax.tick_params('x', colors='black', labelsize=12)
ax.tick_params('y', colors='black', labelsize=12)
ax.set_ylabel('Normalized countrate', rotation=90, fontsize=14, labelpad=10)

ax.plot([(FWHM/2), (FWHM/2)], [0.6, 1.0], linestyle='dashed', color='red', label='FWHM')
ax.plot([(-FWHM/2), (-FWHM/2)], [0.6, 1.0], linestyle='dashed',  color='red')
ax.yaxis.set_ticks([0.6, 1.0])
ax.xaxis.set_ticks([-6, 0, 6])
# plt.grid(True)
plt.legend(loc='best')
# plt.show()
plt.tight_layout()
filename =  f'//fs03/LTH_Neutimag/hkromer/04_Publiations/02_Conferences/2018/IEEE_NSS/Figures/LSF/{my_date}_ESF_fit_LSF'
plt.savefig(filename + '.png', dpi=1200, transparent=False)
plt.close('all')
