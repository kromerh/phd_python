...
for this_folder in folders:
	...
	for file in files:
		...
		dist_min = 10
		dist_max = 90
		df_arrived = df_last[ df_last.iloc[:,0] > dist_,om ]
		...
		this_df_hist['Lplasma'] = Lplasma

		Lplasma = re.findall(r'Lplasma_(.+).csv', file)[0]
		run = this_folder
		df_t = pd.DataFrame([[run, X_FWHM, float(Lplasma)]], columns = ['run', 'FWHM', 'Lplasma'])
		# print(df_t)
		...
		# 2D Histogram
		directory = '{}/{}/2D_histogram_Lplasma_{}'.format(particle_data_path,this_folder,Lplasma)
		...
		# print(times)
		# for mytime in times:
		time_cols = [-1]  # only do last timestep
		for mytime in time_cols:
			...
			c = hist[xidx, yidx]
			p = ax.scatter(x, y, c=c, cmap='jet',s=30, alpha=0.75)
			...
			# plt.tight_layout()
			# plt.show()
			filename =  '{}/{}_2D_histogram_{}_time_{}'.format(directory, nn, Lplasma, mytime)
			# plt.savefig(filename + '.eps', dpi=1200)
			# plt.savefig(filename + '.svg', dpi=1200)
			plt.savefig(filename + '.png', dpi=600)
			plt.close('all')
	...
	df_FWHM = df_FWHM.sort_values(by=['Lplasma'])
	print(df_FWHM)
