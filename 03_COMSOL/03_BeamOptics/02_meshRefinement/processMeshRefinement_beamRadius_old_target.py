...
for file in files:
	...
	# histogram
	num_bins = 100
	n, bins = np.histogram(qr.values, bins=num_bins, density=True)
	...
	ID = re.findall(r'(.+).csv', file)[0]
	mr = re.findall(r'_mr_(.+)_qx', file)[0]
	df_t = pd.DataFrame([[ID, X_FWHM, float(mr)]], columns = ['ID', 'FWHM', 'mr'])
	# print(df_t)
...
ax.plot(df_FWHM['mr'].values, df_FWHM['FWHM'].values)
ax.scatter(df_FWHM['mr'].values, df_FWHM['FWHM'].values)

plt.grid(True)
plt.xlabel('Mesh refinement factor')
plt.ylabel('FWHM [mm]')
# plt.show()
filename =  '{}/mr_beamRadius_FWHM_vs_mr'.format(particle_data_path)
