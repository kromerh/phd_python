...
test.1mm.1mA.0.1eV.2.5e7.particleData.csv']
...
# COMSOL_files = [f for f in COMSOL_files if not 'BIDIR' in f]
# COMSOL_files = [f for f in COMSOL_files if ('06' in f) or ('08' in f)]
...
for COMSOL_data_file in COMSOL_files:
	...
	# print(fname)
	directory = '{}/plots/2D_histograms_timestepping/{}/'.format(COMSOL_data_file_path,fname)
	...
	for mytime in time_cols:
		...
		filename =  '{}/{}.{}'.format(directory, runfile, nn)
		...
		# df_FWHM['id'] = id
		df_FWHM['time'] = mytime
		df_FWHM['run_type'] = runfile
		df_output = df_output.append(df_FWHM)
		nn = nn + 1
		# plt.show()

fname = f'{COMSOL_data_file_path}/plots/2D_histograms_timestepping/df_FWHMs.csv'
