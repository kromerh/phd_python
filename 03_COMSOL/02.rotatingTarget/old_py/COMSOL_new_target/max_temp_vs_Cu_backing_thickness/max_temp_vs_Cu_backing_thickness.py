...
path_to_data_folder[0] = "//fs03/LTH_Neutimag/hkromer/02_Simulations/03_MATLAB/2_target_temperature/run40_lam_CuCrZr"
path_to_data_folder[1] = "//fs03/LTH_Neutimag/hkromer/02_Simulations/03_MATLAB/2_target_temperature/run40_lam_CuOFE"
...
for df in dfs_water_thickness:
	...
	for this_df in [df_CuOFE, df_CuCrZr]:
		if ii % 2 ==  0:
			...
			this_style = '--'
		_, = ax1.plot(this_df['Cu_thickness_mm'], this_df['p_beam_200_W'], this_style, linewidth=2, color=this_color, marker=this_marker, label=this_label, markersize=10)
		lst_plot.append(_)
...
l1 = plt.legend(loc=(0.65,0.54), markerscale=0, fontsize=10,handlelength=1)
...
plt.savefig('max_temp_vs_Cu_backing_thickness.eps', dpi=1200)
plt.savefig('max_temp_vs_Cu_backing_thickness.svg', dpi=1200)
plt.savefig('max_temp_vs_Cu_backing_thickness.png', dpi=1200)
plt.show()
