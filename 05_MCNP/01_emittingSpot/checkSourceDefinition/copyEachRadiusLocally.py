...
dir_runfiles = '//fs03/LTH_Neutimag/hkromer/10_Experiments/02_MCNP/neutron_emitting_spot/2_case_files/_experiment_gauss_20180904/case_0'

# directory where the MCNP run files shall be put into
dir_out = 'D:/neutron_emitting_spotsize/check_source_gauss/'
...
for radius in lst_radii:	
	...
	with open(dst, 'r') as file:
		for line in file:
			# line = line.rstrip()
			t0 = re.findall(r'cd temp_case_0\\(\w+)', line)
			if len(t0) > 0:
