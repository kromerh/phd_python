...
# select directory to scan for ptrac files
master_dir = 'D:/neutron_emitting_spotsize/check_source_gauss_larger/'   # this is from 2018-08-07 MCNP run!

lst_dirs = (os.listdir(master_dir))
lst_dirs = [f for f in lst_dirs if os.path.isdir(f'{master_dir}/{f}')]  # select only directories

df_output = pd.DataFrame()  # contains the mcnp_filename, FWHM_x, FWHM_y

for my_dir in lst_dirs:
	print(f'Doing file: {my_dir}')
	
	# load the ptrac file
	master_folder = f'{master_dir}/{my_dir}'
	

	mcnp_filename = re.findall(r'(rad.+)$',my_dir)[0]
	# print(mcnp_filename)
	this_df = pd.DataFrame()
	this_df['mcnp_filename'] = [mcnp_filename]
	# print(this_df)
	...
	# vals = [1,2,3,4]
	# for val in vals:
	# 	# load the df_ptrac file
	# 	master_folder = f'C:/Users/kromer_h/Downloads/_experiment2018-08-07/FWHM{val}mm/'
	fname = f'{master_folder}/df_ptrac.csv'
	...
	xi, yi = np.mgrid[-0.4:0.4:nbins*1j, -0.4:0.4:nbins*1j]
	...
	this_df['FWHM_x'] = df_FWHM_x['FWHM'].unique()
	...
	this_df['FWHM_y'] = df_FWHM_y['FWHM'].unique()
	...
	df_output = df_output.append(this_df)
