...
ax1.set_xlabel(r'FWHM of LSF in detector [mm]', fontsize= 14)
ax1.set_ylabel(r'FWHM of source definition [mm]', fontsize = 14)

ax1.yaxis.set_ticks([0, 3])
ax1.xaxis.set_ticks([0, 2.5])
# plt.title('Diameter vs FWHM for case {}'.format(case))
# ylims = ax1.get_ylim()
# ax1.set_ylim(0, 1e7)
# fig.subplots_adjust(left=0.15, right=0.97, top=0.88, bottom=0.18)
...
plt.tight_layout()
# plt.show()
filename =  f'//fs03/LTH_Neutimag/hkromer/04_Publiations/02_Conferences/2018/IEEE_NSS/Figures/LSF/'
plt.savefig(f'{filename}/FWHM_LookupTable.png', dpi=1200)
plt.clf()
plt.close('all')

# save the LUT to a file
df.to_csv(f'{filename}/df_LookupTable.csv')
